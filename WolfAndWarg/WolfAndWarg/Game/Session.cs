...
namespace WolfAndWarg
{
    public class Session
    {
        ...
        public void Move(PlayerIndex? controllingPlayer, Vector2 movement)
        {
            ...
            {
                mob.Value.Move(players[playerId].Position, map);
            }
        ...
        public void Draw()
        {
            ...
            foreach (var mob in mobs)
            {
                ...
                hud += string.Format("Warg {0} : {1},", mob.Key, mob.Value.Health);
            }
        ...
        {
            int deadMobCount = mobs.Count(mob => mob.Value.Health <= 0);
